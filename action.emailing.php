...
if(isset($params['id_etudiant']) && $params['id_etudiant'] !='')
{
	$id_presence = $params['id_etudiant'];
	$insc_ops = new T2t_presence;
	$details = $insc_ops->details_presence($id_etudiant);
	$titre = $details['nom'];
	...
	$subject = $this->GetPreference('email_absence_subject');
...
//	$body = $this->ProcessTemplateFromData($body);
	...
	//	$message_id =$db->Insert_ID();
		...
		if ($sent == 1)
		{
			...
			$licences = $pres_ops->relance_email_licence($id_etudiant);
				...
				if(FALSE === $licences || FALSE === in_array($sels, $licences))
				{
					...
					if($dbresult && $dbresult->RecordCount()>0)
					{
						...
						if(!is_null($email_contact))
						{
							...
							$retourid = $this->GetPreference('pageid_etudiant');//44;
							$page = $cg_ops->resolve_alias_or_id($retourid);//'absence';
							$lienok = $this->create_url($id,'default',$page, array("id_etudiant"=>$id_etudiant, "reponse"=>"1","genid"=>$sels));
							$lienko = $this->create_url($id,'default',$page, array("id_etudiant"=>$id_etudiant, "reponse"=>"0","genid"=>$sels));
						//	$lien_recap = $this->create_url($id,'default',$page, array("id_presence"=>$id_presence, "recap"=>"1","genid"=>$sels));
							$montpl = $this->GetTemplateResource('orig_absencemailtemplate.tpl');						
